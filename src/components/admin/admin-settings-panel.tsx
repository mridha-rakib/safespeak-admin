...
const SETTINGS_ITEMS: SettingsItem[] = [
  { label: "Platform Settings", to: APP_ROUTE_PATHS.adminPlatformSettings },
...
export function AdminSettingsPanel() {
  return (
    <div className="w-full min-w-0 rounded-xl border border-[#CAD7E3] bg-white shadow-[0_1px_6px_rgba(0,0,0,0.25)]">
      ...
      <div className="admin-panel-min-h px-4 pb-6 pt-1 sm:px-6">
        <ul>
          {SETTINGS_ITEMS.map((item) => (
            <li key={item.label} className="border-b border-[#C4CFDA]">
              {item.to ? (
                <Link
                  to={item.to}
                  className="flex h-[62px] w-full items-center justify-between text-left text-[22px] font-medium text-[#1E293B] transition hover:bg-[#F7FAFE] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4BA3D9] sm:text-[28px] lg:text-[36px]"
                >
